# Script MikroTik RouterOS 7 - Notificacoes Telegram para Login/Logout
# Versao: 3.0 - Compativel com RouterOS 7
# Data: Marco 2025
...
:if ([:len [/system script find name=$scriptName]] = 0) do={
    ...
    # Criar o script notifica-login-telegram (compativel RouterOS 7)
...
:if ([:len $schedulerExistente] > 0) do={
    ...
    /system scheduler set $schedulerExistente interval=1m on-event=("/system script run " . $scriptName) comment=("Monitoramento de logins via Telegram - Versao 3.0")
} else={
    :log info "Criando novo agendamento $schedulerName..."
    # Usar data/hora atual para inicio
    :local dataInicio [/system clock get date]
    :local horaInicio [/system clock get time]
    
    /system scheduler add name=$schedulerName interval=1m on-event=("/system script run " . $scriptName) policy=read,write,policy,test,password,sensitive comment=("Monitoramento de logins via Telegram - Versao 3.0") start-date=$dataInicio start-time=$horaInicio
}
...
:delay 3s
:local statusScheduler false
:local schedulerItem [/system scheduler find name=$schedulerName]
:if ([:len $schedulerItem] > 0) do={
    :set statusScheduler [/system scheduler get $schedulerItem disabled]
    ...
    :log error "ERRO: Agendamento $schedulerName nao foi criado."
}
